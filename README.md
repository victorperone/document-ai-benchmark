...
**Implementation status: stable and frozen as PyMuPDF Benchmark v2.1 for the initial cross-parser comparison.**
...
- robustness metrics;
- processing-time metrics;
- runtime-selectable artifacts through `--artifacts`;
- optional terminal progress through `--verbose`;
- reproducible OCR regression fixtures;
- OCR quality diagnostics for DPI comparison;
- controlled integration benchmark support.
...
| OCR engine | RapidTess | RapidOCR detection + Tesseract recognition |
| OCR language | `por` | Portuguese-language recognition |
| OCR DPI | 150 | Efficiency-oriented stable default selected after 150/200/300 DPI evaluation |
| Parser header extraction | Enabled | Common normalizer handles removal |
| Parser footer extraction | Enabled | Common normalizer handles removal |
| `force_text` | Enabled | Preserve text even around visual/layout regions |
| Write images | Disabled | Images are not needed in cloud context |
...
### PyMuPDF v2.1 DPI decision

The stable v2.1 profile uses **150 DPI** for automatic OCR.

A controlled DPI ablation compared 150, 200 and 300 DPI on the same image-only OCR fixture. The result showed that 300 DPI did not provide a quality improvement proportional to its computational cost.

Observed extraction/runtime characteristics were:

| DPI | Extraction | Peak RAM | CER | WER |
|---:|---:|---:|---:|---:|
| 150 | 6.771 s | 631.0 MB | 7.90% | 32.86% |
| 200 | 8.108 s | 1011.7 MB | 9.87% | 34.52% |
| 300 | 11.935 s | 2019.9 MB | 6.84% | 31.90% |

The 150 DPI profile therefore became the stable default for the first cross-parser comparison.

The explicit diagnostic profiles remain available:
...
This decision is **not** a claim that 150 DPI is globally optimal. Different document collections may justify different OCR resolutions.
...
- fewer output tokens automatically means better extraction;
- the current OCR profile is optimal for every Portuguese document;
- the current 150 DPI default is globally optimal;
- PyMuPDF will outperform every other parser on every PDF;
- the current Markdown is sufficient for every future RAG question.
...
### Stable v2.1 final regression

Before freezing the adapter, the stable `ocr_auto_rapidtess` profile was executed again with its final 150 DPI configuration.
...
The controlled benchmark final run produced:
...
The controlled document combines native text, tables, formatting, formulas, repeated headers/footers and raster content inside the same PDF. The automatic OCR policy correctly selected only pages 9, 10 and 11 for OCR instead of processing all 12 pages through OCR.
...
### Additional known limitation: rotated raster regions in hybrid PDFs

The controlled benchmark exposed an important orientation edge case. A hybrid PDF page may contain native header/footer text in the correct orientation while an embedded raster region is physically rotated.

The stable PyMuPDF v2.1 pipeline correctly identifies these pages as OCR candidates, and Tesseract OSD was separately validated to detect the orientation of controlled raster regions. However, automatic correction of the rotated raster while preserving its original geometry, resolution and page positioning requires additional region-aware image reconstruction.

The experimental correction was therefore intentionally **not integrated into the stable PyMuPDF adapter**.

```text
△ KNOWN LIMITATION

Physically rotated raster images inside hybrid PDF pages
can be detected successfully with Tesseract OSD, but they
are not automatically corrected in the stable PyMuPDF v2.1
pipeline.
...
This limitation does not block the initial parser comparison. The project will first implement stable versions of PyMuPDF4LLM, Docling, MinerU and PaddleOCR, then use the same controlled document to determine whether another parser handles this scenario natively before additional parser-specific engineering is introduced.
