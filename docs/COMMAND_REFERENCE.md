...
```text
Host OS:       Windows 11 / Windows Server 2025 / Linux
Linux layer:   WSL2 (for Windows) or native Linux
Containers:    Docker Desktop (dev) / Docker Engine (server)
Shell:         Bash (Linux/WSL2) or PowerShell (Windows native)
Repository:    ~/workspace/document-ai-benchmark
```

All repository-relative commands should be executed from the repository root:
...
Current Docker Compose services (Docker-only parsers):
...
Host-only parsers (no Docker image — run via per-parser venv):
...
# 57. Running the full benchmark via run_batch.py

`run_batch.py` is the primary orchestrator. It handles job planning, locking, resume logic, preflight, and post-validation for all parsers.

```bash
# Dry run: print the job plan without executing
python scripts/run_batch.py --suite smoke --dry-run

# Run the smoke suite (all parsers, full_cpu_local profile)
python scripts/run_batch.py --suite smoke

# Run a specific parser + profile combination
python scripts/run_batch.py --parser pymupdf --profile ocr_auto_rapidtess --artifacts all

# Resume an interrupted run (skips jobs that already passed post-validation)
python scripts/run_batch.py --suite ocr_primary --resume

# Preflight check before running (verify models, packages, tesseract)
python scripts/run_batch.py --suite smoke --preflight
```

Named suites are defined in `config/benchmark_profiles.json`:

```text
smoke          — one document × all parsers × full_cpu_local
ocr_primary    — primary OCR profiles across all parsers
full_corpus    — full document corpus × all parsers × all profiles
visual_ablation — compare base vs visual-enrichment profiles
```

---

# 58. Running individual parser adapters directly

Each parser adapter can also be run directly (useful for debugging):
...
# Unstructured via host venv (host-only, no Docker)
python src/parsers/unstructured_v2.py \
  --input data/raw/benchmark_01_simple_18.pdf \
  --output-root outputs \
  --profile full_cpu_local \
  --artifacts all

# XBerg via host venv (host-only, no Docker)
python src/parsers/xberg_v2.py \
  --input data/raw/benchmark_01_simple_18.pdf \
  --output-root outputs \
  --profile full_cpu_local \
  --artifacts all
...
# 59. Parser preflight check

Before running a parser, verify its environment is ready:

```bash
# Check a single parser/profile combination
python scripts/parser_preflight.py --parser pymupdf --profile full_cpu_local

# Check in JSON format (machine-readable)
python scripts/parser_preflight.py --parser docling --profile full_cpu_local --format json
```

The preflight check verifies:
- Required packages are installed at correct versions
- Model artifacts are present (for ML-based parsers)
- Tesseract is available with the required language packs
- Docker image exists (for Docker-only parsers)
...
This is why all parser commands in this document reflect validated, stable implementations.

---

# 67. Current Benchmark v2 adapter status

All seven parsers have completed Benchmark v2 adapters with full Common Core integration.

| Parser | Adapter | Runtime | Status |
|---|---|---|---|
| PyMuPDF | `pymupdf_v2.py` | Docker | ✅ Stable — 8 profiles |
| Docling | `docling_v2.py` | Docker | ✅ Stable — 7 profiles, SmolVLM, TableFormer v1/v2 |
| MinerU | `mineru_v2.py` | Docker | ✅ Stable — 4 profiles |
| PaddleOCR | `paddleocr_v2.py` | Docker | ✅ Stable — 7 profiles, PP-StructureV3 |
| LiteParse | `liteparse_v2.py` | Docker | ✅ Stable — 4 profiles, SmolVLM, Tesseract |
| Unstructured | `unstructured_v2.py` | Host venv | ✅ Stable — 7 profiles, host-only |
| XBerg | `xberg_v2.py` | Host venv | ✅ Stable — 6 profiles, QR code detection, host-only |

All adapters:
- Use `finalize_artifacts()` from Common Core
- Use `process_tree_v2` resource monitoring
- Use `ParserArtifactInput` contract
- Write `metrics.json` schema v3
- Support `--artifacts` and `--verbose` flags
- Are validated by `validate_post_execution()` after each run
