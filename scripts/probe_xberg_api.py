...
async def _run_extraction() -> None:
    """Run a minimal Xberg extraction against a temporary single-page PDF.

    Exercises the async ``xberg.extract`` API with an ``ExtractInput`` and
    ``ExtractionConfig``, then validates the result structure using the
    ``_ok`` / ``_fail`` / ``_warn`` probe helpers.
    """
    try:
        extract_fn = getattr(
            xberg,
